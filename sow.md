...
| Event subscription handlers (6): BugProductChanged → retarget, BugDeleted → cleanup, GroupMemberAdded/Removed → project membership, ProductUpdated, ComponentUpdated → project product/component | Large | 6 | `output/phase-4-architecture/services/service-attachment.md` — Events Consumed table; BugMovedFlagRetargetHandler |
...
| Event subscription handlers (14+): BugCreated, BugUpdated, BugStatusChanged, BugAssigned, BugResolved, BugMarkedDuplicate, CCAdded/Removed, DependencyAdded/Removed, CommentCreated, CommentTagAdded/Removed, AttachmentCreated, AttachmentFlagRequested/Granted/Denied + BugFlagRequested/Granted/Denied (FlagNotificationHandler), EmailPreferencesUpdated, UserWatchingChanged | Large | 7 | `output/phase-4-architecture/services/service-notification.md` — Handler Inventory table |
...
- **R-004 (Resolved 2026-05-06) — Event naming inconsistencies across service architecture docs** [`audit-output/risk-register.md` row R-004]: The four mismatches have been reconciled by aligning on canonical names: `BugProductChanged` (not `BugMoved`), `CommentTagAdded` (not `CommentTagged`), `EmailPreferencesUpdated` (not `UserPreferencesChanged`), and the specific `AttachmentFlagRequested` / `BugFlagRequested` / `AttachmentFlagGranted` / `AttachmentFlagDenied` / `BugFlagGranted` / `BugFlagDenied` events (replacing the generic `FlagSet` / `FlagRequested` aliases). The cross-service contract-lint check planned for Phase 1 still stands as preventive scaffolding so future regressions are caught at build time rather than runtime.
